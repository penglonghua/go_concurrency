...
## select 语句
这个地方第一次遇到的时候，还是很惊讶，后来发现 这个地方是必须选项.

select 语句是将 channel绑定在一起的粘合剂.
channel 是 go 的 粘合剂.


select 语句可以帮助安全地将 channel 与诸如取消，超时，等待和默认之类的概念组合在一起.
...
1 问题， 如果没有任何可用的 channel,会发生什么? 
如果所有的 channel 都被阻塞了, 会发生什么?

如果没有可用的 channel, 那么就是阻塞掉了.
但是不希望永远地阻塞下去，这个时候可以使用 超时机制.
Go语言的 time包提供了一种优雅的方式.

2. 当没有可用的 channel 时，select 也可以有默认值, default.


select 里面的 default 语句，已经 与 for循环的结合使用.



### select {}空语句
这个语句将 永远阻塞., 之前的理解是正确的.

**关于 select 这个地方可以说是 非常重要，可以帮助你安全高效地组合各种概念和子系统**
后面应该还会有专题.
